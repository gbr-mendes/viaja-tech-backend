...
utils.paginatedQuery = async (model, limit = 5, page = 1, resp, queryFields, queryBuilder = null, userFields) => {
    ...
    results.total = total
    results.results = queryBuilder ? await queryBuilder(query, userFields ? userFields : ["-__v", "-password", "-notfications", "-role"]) : query
...
utils.queryBuilderBasedOnUser = async (baseQuery, userFields) => {
    ...
    const results = await Promise.all(baseQuery.map(async item => {
        const { userId } = item
